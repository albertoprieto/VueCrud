 ...
 * @param {object} pago - Objeto completo del pago (con detalle_ordenes, cliente, total, fecha, etc.)
 */
export async function generarPagoPDF(tipo, pago) {
  ...
  const prefijo = esFactura ? 'INV' : 'NV';
  ...
  const tableBody = [
    ...
    ...ordenes.map((o, idx) => [
      ...
      { text: o.nombre_cliente || pago.cliente || '-' },
      { text: o.plataforma || '-' },
      { text: o.usuario || '-' },
  ...
  if (esFactura) {
    totalesRows.push([
      { text: 'IVA (16%)', style: 'totalLabel' },
      { text: `$${iva.toFixed(2)}`, style: 'totalValue' },
    ]);
  }

  totalesRows.push([
    { text: 'Total', style: 'totalLabel' },
    { text: `MXN $${total.toFixed(2)}`, style: 'totalValue' },
  ]);

  const docDefinition = {
    content: [
      // Logo (izquierda) + Titulo y folio (derecha)
      {
        columns: [
          {
            image: logo,
          ...
          {
            stack: [
              { text: titulo, style: 'title', alignment: 'right' },
              { text: folio, style: 'folio', alignment: 'right' },
            ],
            width: '*',
          },
        ],
      ...
      // Info empresa (debajo del logo, a la izquierda)
      {
        stack: [
          { text: empresa.nombre, style: 'empresaHeader' },
          { text: empresa.direccion, style: 'empresaSubheader' },
          { text: `RFC: ${empresa.rfc}`, style: 'empresaSubheader' },
          { text: `Régimen fiscal: ${empresa.regimen}`, style: 'empresaSubheader' },
          { text: `Tel: ${empresa.telefono}`, style: 'empresaSubheader' },
          { text: empresa.correo, style: 'empresaSubheader' },
          { text: empresa.web, style: 'empresaSubheader' },
        ],
        margin: [0, 0, 0, 10],
      },
      { text: '\n' },
        ...
        columns: [
          {
            stack: [
              ...
              { text: pago.cliente || '-', style: 'clienteLabel' },
            ],
            width: '*',
          },
          {
            stack: [
              { text: `Fecha: ${formatFechaPDF(pago.fecha)}`, style: 'clienteLabel', alignment: 'right' },
              { text: `Estatus: ${pago.status || '-'}`, style: 'clienteLabel', alignment: 'right' },
            ],
            width: 'auto',
          },
        ],
      ...
      ordenes.length > 0
        ? {
            table: {
              headerRows: 1,
              widths: [18, 55, 60, 60, 55, 55, 70, 55],
              body: tableBody,
            },
            layout: {
              fillColor: function (rowIndex) {
                return rowIndex === 0 ? '#686868' : rowIndex % 2 === 0 ? '#f3f4f6' : null;
              },
              hLineWidth: () => 0.5,
              vLineWidth: () => 0.5,
              hLineColor: () => '#e5e7eb',
        ...
        : { text: 'Sin órdenes registradas.', style: 'notas', italics: true },
      { text: '\n' },
      // Totales
      {
        columns: [
      ...
      { text: '\n' },
      // IMEIs si existen
      ...(pago.imeis && pago.imeis.length
        ? [
            { text: 'IMEIs asociados', style: 'sectionHeader' },
            { text: pago.imeis.join(', '), style: 'notas' },
            { text: '\n' },
          ]
        : []),
      // Observaciones
      ...
      { text: 'Notas', style: 'sectionHeader' },
      {
        ul: [
          ...
          'Si adquiriste el GPS con recarga mensual, no olvides realizarle su recarga de 50 RECARGA SALDO TELCEL no paquete.',
        ],
        style: 'observaciones',
        margin: [0, 2, 0, 8],
      },
      ...
      {
        ul: [
          'Los equipos GPS tiene un año de garantía contra defectos de fábrica.',
          'Los SIM multicarrier con un año de saldo cuenta con 3 meses de garantía.',
          'Revisiones físicas del técnico tienen costo.',
          'Dispositivos manipulados pierden garantía.',
          'Batería, centro de carga y fusibles del dispositivo no tiene garantía.',
          'La renovación de la plataforma tiene costo al segundo año.',
        ],
        style: 'observaciones',
        margin: [0, 2, 0, 8],
      },
    ...
    styles: {
      ...
      empresaSubheader: { fontSize: 8, color: '#64748b' },
      title: { fontSize: 18, bold: true, color: '#1e293b', margin: [0, 5, 0, 5] },
      folio: { fontSize: 12, color: '#475569', margin: [0, 2, 0, 2] },
      sectionHeader: { fontSize: 11, bold: true, color: '#334155', margin: [0, 8, 0, 4] },
      clienteLabel: { fontSize: 10, color: '#334155', margin: [0, 2, 0, 2] },
      tableHeader: { bold: true, color: '#ffffff', fontSize: 9, alignment: 'center' },
      totalLabel: { bold: true, fontSize: 11, alignment: 'right', color: '#334155', margin: [0, 2, 8, 2] },
      totalValue: { bold: true, fontSize: 11, alignment: 'right', color: '#1e293b', margin: [0, 2, 0, 2] },
      notas: { fontSize: 9, color: '#64748b', margin: [0, 2, 0, 2] },
      observaciones: { fontSize: 8, color: '#475569', margin: [0, 1, 0, 1] },
      disclaimer: { fontSize: 8, color: '#94a3b8', italics: true, margin: [0, 4, 0, 0] },
    },
    defaultStyle: {
      fontSize: 9,
    },
  ...
  const usuario = (ordenes[0]?.usuario || '').replace(/[\\/:*?"<>|]/g, '');
  const clienteNombre = (pago.cliente || '').replace(/[\\/:*?"<>|]/g, '');
  const nombreArchivo = `${folio}_${clienteNombre}_${usuario}.pdf`;
