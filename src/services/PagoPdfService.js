...
const empresa = {
  ...
  direccion: 'Fresno 1441  ·  44910 Guadalajara, Jalisco  ·  RFC: CTR1905206K5',
  rfc: 'CTR1905206K5',
  contacto: 'Tel: 3325373183  ·  gpsvector@gmail.com  ·  gpsubicacion.com',
};
 ...
 * Replica fiel del layout de bot-engine.js (generarNotaPdfBuffer).
 * @param {'nota'|'factura'} tipo
 * @param {object} pago - Objeto completo con detalle_ordenes, cliente, total, fecha, etc.
 */
export async function generarPagoPDF(tipo, pago) {
  ...
  const fechaStr = pago.fecha
    ? pago.fecha.slice(0, 10).split('-').reverse().join('/')
    : new Date().toLocaleDateString('es-MX', { day: '2-digit', month: '2-digit', year: 'numeric' });

  const ordenes      = pago.detalle_ordenes || [];

  // comprobantes puede llegar como array real o como string JSON "[\"...\"]"
  let _comprobantesRaw = pago.comprobantes;
  if (typeof _comprobantesRaw === 'string') {
    try { _comprobantesRaw = JSON.parse(_comprobantesRaw); } catch { _comprobantesRaw = []; }
  }
  const comprobantes = Array.isArray(_comprobantesRaw) ? _comprobantesRaw.filter(Boolean) : [];

  // ── Comprobante preview (nota solamente) ──────────────────────────────────
  ...
  if (tipo === 'nota' && comprobantes.length > 0) {
    try {
      ...
      console.warn('No se pudo previsualizar el comprobante:', err);
    }
  }
  ...
  const tableBody = [
    [
      { text: '#',                style: 'th' },
      { text: 'Orden',            style: 'th' },
      { text: 'Tipo de Servicio', style: 'th' },
      { text: 'IMEI',             style: 'th' },
  ...
  const docDefinition = {
    ...
    content: [
      ...
      // ── Encabezado: logo (izquierda) | título + folio + fecha + estatus (derecha) ──
      {
        columns: [
          { image: LOGO, width: 160, alignment: 'left', margin: [0, 0, 0, 0] },
          {
            stack: [
              { text: titulo,                                  style: 'title'    },
              { text: folio,                                   style: 'folio'    },
              { text: `Fecha: ${fechaStr}`,                    style: 'folioSub' },
              { text: `Estatus: ${pago.status || 'pagado'}`,   style: 'folioSub' },
            ],
            alignment: 'right',
            width: '*',
          },
      ...
      // ── Empresa ──────────────────────────────────────────────────────────
      {
        stack: [
          ...
          { text: empresa.direccion, style: 'sub'     },
          { text: empresa.contacto,  style: 'sub'     },
        ],
        margin: [0, 0, 0, 10],
      },
      ...
      // ── Divisor oscuro ────────────────────────────────────────────────────
      {
        canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1, lineColor: '#334155' }],
        margin: [0, 0, 0, 10],
      },

      // ── Info servicio (izquierda) + Total (derecha) ───────────────────────
      {
        columns: [
          {
            width: '*',
          ...
          },
          {
            width: 'auto',
            stack: [
              { text: esFactura ? 'Total (con IVA)' : 'Total', style: 'totalLabel' },
              {
      ...
      (imeis.length > 0 || ordenes.length > 0)
        ...
        : { text: 'Sin servicios registrados.', italics: true, color: '#64748b' },

      // ── Divisor claro + Condiciones ───────────────────────────────────────
      {
      ...
      ...(tipo === 'nota'
        ? [
            { text: 'Comprobante de pago', style: 'sectionTitle', pageBreak: 'before' },
  ...
  const nombreArchivo = `${folio}_${clienteNombre}.pdf`;
