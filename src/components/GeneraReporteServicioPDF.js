...
// Puedes importar el logo como base64 desde public/logoBase64.txt si lo tienes
...
const empresa = {
  ...
  const empresaFull = {
    ...
    direccion: empresa?.direccion || 'Guadalajara',
    rfc: empresa?.rfc || 'RFC123456',
  ...
  // Minimalist, elegant PDF design
  const docDefinition = {
    content: [
      {
        columns: [
          logoBase64 ? { image: logoBase64, width: 215, alignment: 'left', margin: [0, 0, 0, 10] } : { text: '', width: '*' },
          {
            stack: [
              { text: empresaFull.nombre, style: 'empresaHeader' },
              { text: empresaFull.direccion, style: 'empresaSubheader' },
              { text: empresaFull.rfc, style: 'empresaSubheader' },
              empresaFull.regimen ? { text: `Régimen fiscal: ${empresaFull.regimen}`, style: 'empresaSubheader' } : {},
              empresaFull.telefono ? { text: empresaFull.telefono, style: 'empresaSubheader' } : {},
              empresaFull.correo ? { text: empresaFull.correo, style: 'empresaSubheader' } : {},
              empresaFull.web ? { text: empresaFull.web, style: 'empresaSubheader' } : {}
            ].filter(Boolean)
          }
        ],
        margin: [0, 0, 0, 8],
      ...
      { canvas: [ { type: 'line', x1: 0, y1: 0, x2: 520, y2: 0, lineWidth: 1, lineColor: '#bbb' } ], margin: [0, 4, 0, 12] },
      {
        columns: [
          { text: 'REPORTE DE SERVICIO', style: 'folio', alignment: 'left' },
          { text: `Folio: ${folio}`, style: 'folio', alignment: 'right' },
          { text: `Fecha: ${fechaStr}`, style: 'folio', alignment: 'right' }
        ],
        margin: [0, 0, 0, 8]
      },
      {
        columns: [
          {
            ...
            stack: [
              ...
              { text: reporte.vendedor || '', style: 'cardValue' }
            ],
            margin: [0,0,8,0],
            fillColor: '#fafafa',
            border: [false, false, false, false]
          },
          {
            width: '33%',
            stack: [
              { text: 'Tipo de Servicio', style: 'cardHeader' },
              { text: reporte.tipo_servicio || '', style: 'cardValue' },
              { text: 'Lugar Instalación', style: 'cardHeader' },
              { text: reporte.lugar_instalacion || '', style: 'cardValue' },
                ...
                table: {
                  ...
                  body: [
                    [
                      ...
                      { text: 'Submarca', style: 'tableHeader' },
                    ...
                    [
                      { text: 'Placas', style: 'tableHeader' },
                      { text: 'Color', style: 'tableHeader' },
                      { text: 'No. Económico', style: 'tableHeader' }
                    ],
                    [
                      reporte.placas || '',
                      reporte.color || '',
                      reporte.numero_economico || ''
                    ]
                  ]
                },
                layout: {
        ...
        columns: [
          {
            ...
            stack: [
              ...
              { text: reporte.observaciones || '', style: 'notas' }
            ],
            fillColor: '#fafafa',
            border: [false, false, false, false]
          },
    ...
    styles: {
      ...
      empresaSubheader: { fontSize: 10, color: '#666', margin: [0, 0, 0, 1] },
      title: { fontSize: 16, bold: true, color: '#444', margin: [0, 8, 0, 10] },
      sectionHeader: { fontSize: 12, bold: true, color: '#444', margin: [0, 8, 0, 2] },
      cardHeader: { fontSize: 11, bold: true, color: '#444', margin: [0, 4, 0, 0] },
      cardValue: { fontSize: 10, color: '#444', margin: [0, 0, 0, 4] },
      tableHeader: { bold: true, fillColor: '#eeeeee', color: '#444', fontSize: 10, alignment: 'center' },
      totalLabel: { bold: true, fontSize: 11, alignment: 'right', color: '#444' },
