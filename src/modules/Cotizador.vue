...
const clienteEmails = computed(() => {
  ...
  return cliente && cliente.emails ? cliente.emails : [];
});
watch(
  () => cotizacion.value.cliente_id,
  (nuevoId) => {
    const emails = clienteEmails.value;
    cotizacionEmail.value = emails.length > 0 ? emails[0] : '';
  }
);
    ...
    <div class="cotizador-card">
      <form @submit.prevent="guardarCotizacion">
        <div class="form-row">
          ...
          <div class="form-group">
            <label>Emiliano Zapata</label>
            <Dropdown
              v-model="cotizacionEmail"
              :options="clienteEmails"
              placeholder="Selecciona Emiliano Zapata"
              class="w-full"
              :disabled="clienteEmails.length === 0"
            />
          </div>
          <div class="form-group">
            <label>Fecha</label>
        ...
        <div class="actions-right">
          ...
          <Button label="Generar PDF" icon="pi pi-file-pdf" class="p-button-secondary ml-2" @click="generarPDFCotizacion" />
        </div>
      </form>
      ...
      <Dialog v-model:visible="showSendDialog" header="Enviar Cotización" :closable="false" :modal="true">
        ...
        <p><strong>Emiliano Zapata:</strong> {{ cotizacionEmail }}</p>
        <template #footer>
          <Button label="Cancelar" @click="showSendDialog = false" />
